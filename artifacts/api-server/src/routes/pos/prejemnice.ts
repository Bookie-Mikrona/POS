...
async function nextStevilkaPrejemnica(year: number, _davcna: string, tenotaId: number): Promise<string> {
  ...
  const result = await db.execute(
    sql`SELECT COALESCE(MAX(CAST(SUBSTRING(stevilka, 3) AS INTEGER)), 0) + 1 AS next FROM prejemnice WHERE stevilka LIKE ${`${yy}%`} AND LENGTH(stevilka) = 8 AND podjetje_davcna = ${""} AND enota_id = ${tenotaId}`
  );
