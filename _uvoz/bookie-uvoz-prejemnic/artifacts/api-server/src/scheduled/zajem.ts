...
async function obdelajSporocilo(
  ...
  const [znan] = await db.execute<{ partner_id: number }>(sql`
    SELECT partner_id FROM partner_eposta
     WHERE podjetje_id = ${p.podjetje_id}
  ...
  for (const priloga of urejene) {
    try {
      ...
      const dob = await dolociDobavitelja(db, p.podjetje_id, dto, posiljatelj);
...
async function obdelajCakajoceSeje(): Promise<void> {
  const seje = await db.execute<{ id: string; podjetje_id: number }>(sql`
    SELECT id, podjetje_id FROM uvoz_seja
  ...
  for (const s of seje) {
    try {
      ...
      const dob = await dolociDobavitelja(db, s.podjetje_id, dto);
      if (!dob.dobaviteljId) continue;

      await ustvariOsnutek(db, {
        podjetjeId: s.podjetje_id,
        sejaId: s.id,
        dobaviteljId: dob.dobaviteljId,
        dto,
        uporabnikId: SISTEMSKI_UPORABNIK,
...
async function glavni(): Promise<void> {
  ...
  const predali = await db.execute<Predal>(sql`
    SELECT id, podjetje_id, enota_id, streznik, vrata, uporabnisko_ime,
  ...
  for (const p of predali) {
    await zZaklepom(`zajem_eposta:${p.id}`, async () => {
      try {
