...
router.get('/profili', zahtevajPravico('PREVZEM_VNOS'), async (req: AvtoriziranRequest, res) => {
  const q = z
...
router.post('/profili', zahtevajPravico('PREVZEM_VNOS'), async (req: AvtoriziranRequest, res) => {
  ...
  const [p] = await db.execute<{ id: number }>(sql`
    ...
    VALUES (
      ${req.uporabnik.podjetjeId}, ${v.data.dobaviteljId}, ${v.data.naziv},
...
router.put('/profili/:id', zahtevajPravico('PREVZEM_VNOS'), async (req: AvtoriziranRequest, res) => {
...
router.post(
  ...
  async (req: AvtoriziranRequest, res) => {
    ...
    const predogled = await predogledCenika(
      db, req.uporabnik.podjetjeId, vhod.data.dobaviteljId, vrstice,
    );
  ...
  async (req: AvtoriziranRequest, res) => {
    ...
    const vrstice = await db.execute(sql`
      SELECT ad.id, ad.sifra_dobavitelja, ad.gtin, ad.naziv_dobavitelja,
       ...
       WHERE ad.podjetje_id = ${req.uporabnik.podjetjeId}
