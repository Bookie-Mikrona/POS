...
import { db } from '../../db';
import { zahtevajPravico, type AvtoriziranRequest } from '../../middleware/avtorizacija';
import {
  dolociDobavitelja,
  razcleniSejo,
  ustvariOsnutek,
...
router.post(
  '/datoteka',
  zahtevajPravico('PREVZEM_VNOS'),
  nalozi.single('datoteka'),
  async (req: AvtoriziranRequest, res: Response) => {
    if (!req.file) {
    ...
    try {
      const izid = await uvozi(db, {
        podjetjeId: req.uporabnik.podjetjeId,
        enotaId: vhod.data.enotaId ?? req.uporabnik.enotaId,
        kanal: 'ROCNI_NALOG',
        raw: req.file.buffer,
...
router.get('/seje', zahtevajPravico('PREVZEM_VNOS'), async (req: AvtoriziranRequest, res) => {
  const q = z
    .object({
  ...
  const vrstice = await db.execute(sql`
    SELECT s.id, s.kanal, s.format, s.ime_datoteke, s.velikost_b,
     ...
     WHERE s.podjetje_id = ${req.uporabnik.podjetjeId}
...
router.get(
  '/seje/:id/izvirnik',
  zahtevajPravico('PREVZEM_VNOS'),
  async (req: AvtoriziranRequest, res) => {
    const [s] = await db.execute<{
      ...
      SELECT vsebina, ime_datoteke, mime_tip FROM uvoz_seja
       WHERE id = ${req.params.id} AND podjetje_id = ${req.uporabnik.podjetjeId}
    `);
...
router.post(
  ...
  async (req: AvtoriziranRequest, res) => {
    ...
    try {
      ...
      const dto = await razcleniSejo(db, req.params.id);
      ...
      const dob = await dolociDobavitelja(
        db, req.uporabnik.podjetjeId, dto, null, vhod.dobaviteljId ?? null,
      );
      if (!dob.dobaviteljId) {
      ...
      const izid = await ustvariOsnutek(db, {
        podjetjeId: req.uporabnik.podjetjeId,
        enotaId: req.uporabnik.enotaId,
        sejaId: req.params.id,
...
router.get(
  ...
  zahtevajPravico('PREVZEM_UPARJANJE'),
  async (req: AvtoriziranRequest, res) => {
    const vrstice = await db.execute(sql`
      SELECT pp.id, pp.zap_st, pp.izv_gtin, pp.izv_sifra, pp.izv_naziv,
       ...
       WHERE pp.prejemnica_id = ${req.params.id}
         AND p.podjetje_id = ${req.uporabnik.podjetjeId}
...
router.post(
  ...
  async (req: AvtoriziranRequest, res) => {
    const izid = await upariPrejemnico(
      db, req.uporabnik.podjetjeId, Number(req.params.id),
    );
  ...
  async (req: AvtoriziranRequest, res) => {
    ...
    const vrstice = await db.execute(sql`
      SELECT p.id AS prejemnica_id, p.stevilka, p.datum, p.st_dokumenta,
       ...
       WHERE p.podjetje_id = ${req.uporabnik.podjetjeId}
