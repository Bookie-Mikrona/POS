...
-- razredom, delni indeksi) je treba dodati ročno. Postopek:
--
--   1. drizzle-kit generate            -> ustvari osnutek z DDL tabel
--   2. razdelek 1 prilepi PRED tabele  -> funkcije morajo obstajati,
--                                          preden nastane generirani stolpec
--   3. razdelka 5 in 6 dodaj na konec
--
...
-- Nabavna stopnja je ločena od prodajne davčne kategorije: kavna zrna se
...
COMMENT ON COLUMN artikli.nabavna_ddv_stopnja IS
  'Pričakovana stopnja DDV na prejemnici. Neodvisna od prodajne davčne
   kategorije. Napolni se ob prvem prevzemu iz dokumenta, nato služi
   kot kontrola.';
...
CREATE TABLE IF NOT EXISTS artikel_dobavitelj (
    ...
    artikel_id          bigint NOT NULL REFERENCES artikli(id),
    dobavitelj_id       bigint NOT NULL REFERENCES partnerji(id),
...
CREATE TABLE IF NOT EXISTS artikel_pakiranje (
    id             bigserial PRIMARY KEY,
    podjetje_id    bigint NOT NULL,
    artikel_id     bigint NOT NULL REFERENCES artikli(id) ON DELETE CASCADE,
    naziv          text NOT NULL,
...
CREATE TABLE IF NOT EXISTS uvoz_seja (
    ...
    napake         jsonb,
    metapodatki    jsonb,
    prejemnica_id  bigint REFERENCES prejemnice(id),
...
CREATE TABLE IF NOT EXISTS partner_eposta (
    id               bigserial PRIMARY KEY,
    podjetje_id      bigint NOT NULL,
    partner_id       bigint NOT NULL REFERENCES partnerji(id) ON DELETE CASCADE,
    naslov           text,
    domena           text,
    zaupanja_vreden  boolean NOT NULL DEFAULT false,
    dodal_uporabnik  bigint,
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_ad_sifra
    ON artikel_dobavitelj (podjetje_id, dobavitelj_id, sifra_dobavitelja)
    WHERE sifra_dobavitelja IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS uq_ad_gtin
    ON artikel_dobavitelj (podjetje_id, dobavitelj_id, gtin)
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_seja_hash
    ON uvoz_seja (podjetje_id, sha256);
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_pe_naslov
    ON partner_eposta (podjetje_id, lower(naslov)) WHERE naslov IS NOT NULL;
...
-- Deluje samo, kadar je številka vpisana — obstoječi zapisi ostanejo veljavni.
CREATE UNIQUE INDEX IF NOT EXISTS uq_prejemnica_dokument
    ON prejemnice (podjetje_id, dobavitelj_id, st_dokumenta, datum_dokumenta)
    WHERE st_dokumenta IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS ux_artikel_gtin
    ON artikli (podjetje_id, gtin) WHERE gtin IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS ix_artikel_naziv_trgm
    ON artikli USING gin (public.naziv_norm(naziv) gin_trgm_ops);
